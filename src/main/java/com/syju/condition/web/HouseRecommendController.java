...
@Controller
@RequestMapping("/houseRecommend")
public class HouseRecommendController extends BaseController {
	...
	public String lsit(@RequestParam(value = "page", defaultValue = "1") int pageNumber,
		...
		Page<HouseRecommend> houseRecommends = houseRecommendService.getHouseRecommend(pageNumber, pageSize, sortType,
				searchParams);
	...
	public void move(@RequestParam(value = "id", defaultValue = "") Long id,
		...
		houseRecommendService.move(id, type);
	}

	@RequestMapping(value = "delete/{id}")
	public String delete(@PathVariable("id") Long id, RedirectAttributes redirectAttributes) {
		HouseRecommend featureHouse = houseRecommendService.getHouseRecommend(id);
		Long priority = featureHouse.getPriority(); // 获取删除的分类排序号
		houseRecommendService.deleteHouseRecommend(id);

		houseRecommendService.moveSorting(priority); // 删除后后，批量修改排序号（-1）
	 ...
	 */
	@ModelAttribute
	public void getHouseRecommend(@RequestParam(value = "id", defaultValue = "-1") Long id, Model model) {
		if (id != -1) {
			HouseRecommend featureHouse = houseRecommendService.getHouseRecommend(id);
