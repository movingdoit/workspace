...
import com.syju.house.service.HouseTypePhotoService;
import com.syju.house.service.HouseService;

@Controller
@RequestMapping(value = "/house/phototype")
public class HouseTypePhotoController extends BaseController {

	@Resource
	private HouseTypePhotoService houseTypePhotoService;
	...
	public String createForm(Model model) {
		...
		model.addAttribute("photoType",DictType.getHouseTypePhoto());
		model.addAttribute("action", "create");
		return "house/housetypephoto/photoForm";
	}
	...
	@ResponseBody
	public Map<String, String> createForm(HouseTypePhoto photo,Long houseId,HttpServletRequest request,
		...
		if(houseId==null){
			...
			try{
				...
				// 处理上传图片
				 ...
				 houseTypePhotoService.savePhoto(photo);
			}catch(Exception e){
				code="1111";
				message = "保存失败,请重试";
			}
	...
	public String createForm(Model model,@PathVariable("id") Long id) {
		...
		HouseTypePhoto photo = houseTypePhotoService.findById(id);
	...
	@ResponseBody
	public Map<String, String> updateForm(HouseTypePhoto photo,Long houseId,HttpServletRequest request,
					@RequestParam(value = "file", required = false) MultipartFile file	) {
		Map<String, String> resultMap = new HashMap<String, String>();
		String code = "0000";
		String message = "修改成功";
	...
	public String houseSearch(Model model, ServletRequest request,
		...
		Page<HouseTypePhoto> photePage = houseTypePhotoService.findByNameAndType(searchParams, pageNumber, pageSize, sortType);
		
		
		model.addAttribute("sortType", sortType);
		// 将搜索条件编码成字符串，用于排序，分页的URL
		model.addAttribute("search", searchParams);
		model.addAttribute("type", DictType.getHouseTypePhoto());
		model.addAttribute("photoPage",photePage);
		model.addAttribute("searchParams", Servlets.encodeParameterStringWithPrefix(searchParams, "search_"));
	
		return "house/housetypephoto/photoList";
	}
	...
	public List<HouseInfo> getHouse( @PathVariable("type") int type){
		List<HouseInfo> houseList = houseTypePhotoService.getHouseByType(type);
		return houseList;
	}
	...
	public Map<String, String> deleteById(@PathVariable("id") Long id){
		...
		if(!houseTypePhotoService.deleteById(id)){
			code = "1111";
			message ="删除失败,请重试";
		}
		resultMap.put("code", code);
