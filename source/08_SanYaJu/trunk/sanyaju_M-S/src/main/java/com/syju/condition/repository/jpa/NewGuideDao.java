...
public interface NewGuideDao extends PagingAndSortingRepository<NewGuide, Long>, JpaSpecificationExecutor<NewGuide> {

	// 验证排序号是否存在
	NewGuide findByPriority(Long priority);

	// 批量修改 排序号 (插入排序号)
	@Modifying
	@Query("update NewGuide ng set ng.priority = ng.priority+1 where ng.priority >= ?1")
	...
	@Modifying
	@Query("update NewGuide ng set ng.priority = ng.priority-1 where ng.priority > ?1 and ng.priority <= ?2")
	...
	@Query("select MAX(priority) from NewGuide where priority < (select priority from NewGuide where id=?1)")
	Long getMaxIndex(Long id);

	// 根据ID查询出排序号，然后找出大于当前排序号的最小排序号（用来下移）
	@Query("select MIN(priority) from NewGuide where priority > (select priority from NewGuide where id=?1)")
	...
	// 获取最大排序号
	@Query("select MAX(priority) from NewGuide ")
	Long getMaxPriority();

	// 通过外键查找
	NewGuide findByHouseInfoId(Long id);
	/**
	 * 通过外检关联查询
	 * 
	 * @param name
	 ...
	 * @return
	 */
	@Query("select s from NewGuide s,HouseInfo h where s.houseInfo.id = h.id and h.name like ?1 ")
	Page<NewGuide> findNewGuideAndHouse(String name, Pageable pageable);
	 ...
	 */
	@Modifying
	@Query("delete from  NewGuide sh  where sh.houseInfo.id = ?1")
