...
public class NewGuideController extends BaseController {
	...
	public String lsit(@RequestParam(value = "page", defaultValue = "1") int pageNumber,
		...
		Page<NewGuide> newGuides = newGuideService.getNewGuide(pageNumber, pageSize, sortType, searchParams);
	...
	public void move(@RequestParam(value = "id", defaultValue = "") Long id,
		...
		newGuideService.move(id, type);
	}

	@RequestMapping(value = "delete/{id}")
	public String delete(@PathVariable("id") Long id, RedirectAttributes redirectAttributes) {
		NewGuide featureHouse = newGuideService.getNewGuide(id);
		Long priority = featureHouse.getPriority(); // 获取删除的分类排序号
		newGuideService.deleteNewGuide(id);

		newGuideService.moveSorting(priority); // 删除后后，批量修改排序号（-1）
	 ...
	 */
	@ModelAttribute
	public void getNewGuide(@RequestParam(value = "id", defaultValue = "-1") Long id, Model model) {
		if (id != -1) {
			NewGuide featureHouse = newGuideService.getNewGuide(id);
