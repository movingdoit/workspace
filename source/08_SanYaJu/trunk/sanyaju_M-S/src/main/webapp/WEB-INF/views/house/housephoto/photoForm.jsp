...
<html>
<head>
<title>${action == 'create' ? '添加' : '编辑' }楼盘相册</title>
...
<body>
	...
	<div id="content">
		...
		<div id="content-header">
			<h1>
				<i class="icon-edit"></i> ${action == 'create' ? '添加' : '编辑' }楼盘相册
			</h1>
		</div>
		<!-- // Breadcrumbs -->
		<!-- Action Boxes -->
				...
				<div class="span12">
					<div class="widget-box">
						<div class="widget-title"> <span class="icon"> <i class="icon-edit"></i> </span>
	                        <h5> ${action == 'create' ? '添加' : '编辑' }楼盘相册</h5>
	                        <p style="margin: 0px;">
	                           <a class="btn" href="${ctx}/house/photo/search"><i class="icon-plus"></i>相册列表</a>
	                        </p>
	                    </div>

						<div class="widget-content nopadding">
							<form id="photoForm" action="/house/photo/${action }"
								...
								<div class="control-group">
									<label class="control-label"><i class="red">*</i>  图片类型 :</label>
	                                <div class="controls">
	                                   <select name="photoType" class="input-medium" id="cmsContent_channel" data-rule-required="true" >
	                                        <c:forEach items="${photoType}" var="type">
	                                          <c:choose>
	                                             <c:when test="${ photo.photoType == type.key }">
	...
	<script>
		...
		$("#save").click(function() {
			if ($("#photoForm").valid()) {
				$("#photoForm").ajaxSubmit(function(data) { //提交成功的回调函数  
					if (data.code == "0000") {
						alert(data.message);
						location.href = "${ctx}/house/photo/create";
						//location.reload();
					} else {
						alert("操作异常，请重试。");
					}
		...
		function getHouse(){
			...
			$.post("${ctx}/house/photo/gethouse/"+id,function(data){
				if(data){
