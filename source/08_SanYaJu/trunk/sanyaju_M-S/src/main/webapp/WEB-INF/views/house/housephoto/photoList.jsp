...
<body>
		...
		<div class="container-fluid">
			...
			<div class="row-fluid">
				<form class="form-inline pt15 pb15"
					action="${ctx}/house/photo/search" method="get"
							...
							<c:if test="${not empty photoPage && photoPage.size >0 }">
								<c:forEach items="${photoPage.content}" var="photo">
									<tr>
										...
										<td>
										   <c:forEach items="${type}" var="type">
										   	   <c:if test="${type.key == photo.photoType}">
										   	   		${type.value}
										   	   </c:if>
										   </c:forEach>
										</td>
										...
										<td>
										  <a href="${ctx}/house/photo/update/${photo.id}" class="btn btn-mini">编辑</a> 
	...
	<script type="text/javascript">
	     function del(id){
	    	 if(confirm("确认删除吗？")){
	    		 $.post("${ctx}/house/photo/delete/"+id,function(data){
	    			 debugger;
	 				alert(data.message);
	 				location.href = "${ctx}/house/photo/search";
	 			});
	    	 }
	     }
