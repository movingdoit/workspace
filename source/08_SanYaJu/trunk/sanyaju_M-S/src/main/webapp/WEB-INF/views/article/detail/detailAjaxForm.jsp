...
<form id="articleForm" modelAttribute="article" action="#" method="post"
	class="form-horizontal form-validate" enctype="multipart/form-data">
	<input type="hidden" name="id" value="${article.id}" /> <input
		type="hidden" name="custom" value="${custom}" /> <input type="hidden"
		name="manager" value="${manager}" /> <input type="hidden"
		name="status" value="${article.status != null ? article.status : 1}" />
	<div class="modal-header">
		<button type="button" class="close" data-dismiss="modal"
			onclick="location.reload();" aria-hidden="true">×</button>
	...
	<div class="modal-body">
		<div class="control-group">
			<c:if
				test="${custom == 1 && ((null != articleCategory && articleCategory.id > 1) || (null != article.articleCategory && article.articleCategory.id >1)) }">
				<input type="hidden" name="articleCategory.id"
					value="${articleCategory.id}${article.articleCategory.id}" />
			</c:if>
			<c:if test="${custom != 1}">
				<label class="control-label"><i class="red">*</i> 所属分类 :</label>
				<div class="controls">
					<select name="articleCategory.id" class="input-medium"
						id="article_channel" data-rule-required="true">
						<option value="">选择文章类别</option>
						<c:forEach items="${articleCategorys}" var="articleCategory">
							<option value="${articleCategory.id }"
								${article.articleCategory.id == articleCategory.id ?  'selected' :'' }>${articleCategory.name }</option>
						</c:forEach>
					</select> <span class="help-inline"><a
						href="${ctx}/cms/channel/create?pid=${pid}" data-toggle="modal"
						class="add-cat"><i class="icon-plus"></i> 新增分类</a></span>
				</div>
			</c:if>
		</div>
		...
		<div class="control-group">
			<c:if test="${action == 'create'}">
				...
				<div class="controls">
					<select name="recommendType" onchange="getHouse()"
						class="input-medium" id="recommendType" data-rule-required="true">
			...
			<c:if test="${not empty article.houseInfo}">
				...
				<div class="controls">
					<input type="text" id="name" name="name" disabled="disabled"
						value="${photo.houseInfo.name}" class="span3">
				</div>
			</c:if>
		</div>
		...
		<div class="control-group">
			...
			<div class="controls">
				<input type="text" id="article_title" name="title"
					value="${article.title}" placeholder="最多可输入20字"
					class="input-xlarge" data-rule-required="true"
					data-rule-maxlength="20"> <span class="maroon">*</span>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">显示顺序：</label>
			<div class="controls">
				<input type="text" class="input-small" id="article_priority"
					name="priority" value="${article.priority}"
					data-rule-required="true" data-rule-number="true"> <span
					class="help-inline">数值越大，位置越靠前</span>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				<label class="radio inline"><input type="radio"
					name="isDisplay" value="true" ${article.isDisplay ? 'checked' : ''}>显示</label>
				<label class="radio inline"><input type="radio"
					name="isDisplay" value="false"
					${!article.isDisplay ? 'checked' : ''}>隐藏</label>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				<div class="fileupload fileupload-new" data-provides="fileupload">
					<div class="fileupload-new thumbnail"
						...
						<img
							src="${(null != article && article.coversPic != null && article.coversPic != '') ? article.coversPic : '/static/assets/img/noimage.gif'}" />
					</div>
					<div class="fileupload-preview fileupload-exists thumbnail"
						style="max-width: 200px; max-height: 150px; line-height: 20px;"></div>
					<div>
						<span class="btn btn-file"><span class="fileupload-new">选择图片</span><span
							class="fileupload-exists">更换</span><input type="file" name='file' /></span>
		...
		<div class="control-group">
			...
			<div class="controls">
				<textarea id="article_summary" name="summary" rows="5"
					class="input-block-level">${article.summary}</textarea>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">正文：</label>
			<div class="controls">
				<textarea id="article_content" name="articleContent.content"
					rows="15" class="input-block-level" data-rule-required="true">${null != article.articleContent.content && 'null' != article.articleContent.content ? article.articleContent.content : ''}</textarea>
...
<script src="/static/plug-in/KindEditor/kindeditor.js"
	type="text/javascript"></script>
<script src="/static/plug-in/KindEditor/lang/zh_CN.js"
	type="text/javascript"></script>
...
<script>
	...
	var editor;
	var editorItems = [ 'source', '|', 'justifyleft', 'justifycenter',
